document.addEventListener("DOMContentLoaded", async () => {
  ...
  const typeIcons = {
    video: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e847c8cc80d0e3b76865_video.svg" alt="" width="16" height="16">',
    text: "📝",
    quiz: "❓",
    quiz_wheel: "🎡",
    quiz_choice: "✅",
    pause: "☕",
    aufgabe: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e84780afed7135c57159_aufgabe%3ARreflexion.svg" alt="" width="16" height="16">',
    basiskarte: "🃏",
    checkliste: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e8470a6d48120bf7ab93_checkliste.svg" alt="" width="16" height="16">',
    checklist: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e8470a6d48120bf7ab93_checkliste.svg" alt="" width="16" height="16">',
    audio: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e847f6df29178f156aa4_audio.svg" alt="" width="16" height="16">',
    reflection: '<img src="https://cdn.prod.website-files.com/68ecc710e558140c0c35a381/6943e84780afed7135c57159_aufgabe%3ARreflexion.svg" alt="" width="16" height="16">',
    fazit: "📊",
    abschluss: "🎓",
    welcome: "👋",
  };
  ...
  async function showBadgeModal(badge) {
    ...
    const badgeIds = courseBadges.map(b => b.badgeId);
    const { data: earnedBadgesData } = await db
      .from("user_badges")
      .select("badge_id")
      .eq("user_id", userId)
      .in("badge_id", badgeIds);
    const earnedBadgeIds = new Set(earnedBadgesData?.map(b => String(b.badge_id)) || []);
    
    // Badge Bilder laden für bereits verdiente
    let badgeImages = {};
    if (earnedBadgesData && earnedBadgesData.length > 0) {
      const { data: badgesData } = await db
        .from("badges")
        .select("id, image")
        .in("id", badgeIds);
      if (badgesData) {
        badgesData.forEach(b => { badgeImages[String(b.id)] = b.image; });
      }
    }
    ...
    const currentBadgeImgSrc = `${DIRECTUS_URL}/assets/${badge.image}`;
      ...
      placeholdersHTML = `
        <div class="badge-placeholder active">
          <div class="badge-slot">
            ...
            <img class="badge-earned-img animate-in" src="${currentBadgeImgSrc}" alt="${badge.title}">
          </div>
          <span class="badge-placeholder-label">Modul 01</span>
        </div>`;
    } else {
      courseBadges.forEach((b, i) => {
        const isCurrentBadge = i === currentBadgeIndex;
        const isEarned = earnedBadgeIds.has(String(b.badgeId));
        const badgeImg = badgeImages[String(b.badgeId)];
        
        let badgeImgHTML = '';
        if (isCurrentBadge) {
          // Aktuelles Badge - mit Animation
          badgeImgHTML = `<img class="badge-earned-img" src="${currentBadgeImgSrc}" alt="${badge.title}">`;
        } else if (isEarned && badgeImg) {
          // Bereits verdientes Badge - ohne Animation
          badgeImgHTML = `<img class="badge-earned-img already-earned" src="${DIRECTUS_URL}/assets/${badgeImg}" alt="">`;
        }
        
        placeholdersHTML += `
          <div class="badge-placeholder ${isCurrentBadge ? 'active' : ''} ${isEarned && !isCurrentBadge ? 'earned' : ''}">
            <div class="badge-slot">
              <img class="badge-placeholder-img" src="${BADGE_PLACEHOLDER_URL}" alt="">
              ${badgeImgHTML}
            </div>
